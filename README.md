...
Welcome to ZeepinChain's source code repository!

ZeepinChain is dedicated to creating a modularized, freely configurable, interoperable cross-chain, high-performance, and horizontally scalable blockchain infrastructure system. ZeepinChain makes deploying and invoking decentralized applications easier.
...
The public test network is described below. We sincerely welcome and hope more developers join ZeepinChain.
...
- [Get ZeepinChain](#get-ZeepinChain)
...
- `tools/sigsvr`: (optional)ZeepinChain Signature Server - sigsvr is a rpc server for signing transactions for some special requirement.detail docs can be reference at [link](./docs/specifications/sigsvr.md)
...
## Server deployment
### Select network
To run ZeepinChain successfully,  nodes can be deployed by three ways:
...
Run command `$ ./ZeepinChain --testmode --networkid 3` can start single-host test net.
...
Note: When you want to build a private net to run ZeepinChain in DBFT or VBFT, you must use --config argument to specify a configuration file, and
...
In the multi-hosts enviroment, we need 7 nodes to run ZeepinChain at least in VBFT.
...
Run `./ZeepinChain --help` for details, also you can read [ZeepinChain CLI user guide](./docs/specifications/cli_user_guide.md) to get more information.
...
1. make docker image
    ...
    - In the root directory of source code，run`make docker`, it will make ZeepinChain image in docker.

2. run ZeepinChain image

    - Use command `docker run zeepin/ZeepinChain`to run ZeepinChain；

    - If you need to allow interactive keyboard input while the image is running, you can use the `docker -ti run zeepin/ZeepinChain` command to start the image;
    ...
    - If you need to add ZeepinChain parameters, you can add them directly after `docker run zeepin/ZeepinChain` such as `docker run zeepin/ZeepinChain --networkid 2`.
     The parameters of ZeepinChain command line refer to [here](./docs/specifications/cli_user_guide.md).

### ZPT transfer sample
 -- from: transfer from； -- to: transfer to； -- amount: zpt amount；
```shell
  ./ZeepinChain asset transfer  --to=AXkDGfr9thEqWmCKpTtQYaazJRwQzH48eC --amount=10
```
...
```shell
./ZeepinChain asset balance --address=AYiToLDT2yZuNs3PZieXcdTpyC5VWQmfaN
```
result：
```shell
...
Please open a pull request with a signed commit. We appreciate your help! You can also send your code as emails to the developer mailing list. You're welcome to join the ZeepinChain mailing list or developer forum.
...
The ZeepinChain library is licensed under the GNU Lesser General Public License v3.0, read the LICENSE file in the root directory of the project for details.
