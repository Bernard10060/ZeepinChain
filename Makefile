...
ZeepinChain: $(SRC_FILES)
	$(GC)  $(BUILD_NODE_PAR) -o ZeepinChain main.go
...
all: ZeepinChain tools

ZeepinChain-cross: ZeepinChain-windows ZeepinChain-linux ZeepinChain-darwin

ZeepinChain-windows:
	CGO_ENABLED=0 GOOS=windows GOARCH=amd64 $(GC) $(BUILD_NODE_PAR) -o ZeepinChain-windows-amd64.exe main.go

ZeepinChain-linux:
	CGO_ENABLED=0 GOOS=linux GOARCH=amd64 $(GC) $(BUILD_NODE_PAR) -o ZeepinChain-linux-amd64 main.go

ZeepinChain-darwin:
	CGO_ENABLED=0 GOOS=darwin GOARCH=amd64 $(GC) $(BUILD_NODE_PAR) -o ZeepinChain-darwin-amd64 main.go
...
all-cross: ZeepinChain-cross tools-cross abi
...
docker/payload: docker/build/bin/ZeepinChain docker/Dockerfile $(ZPT_CFG_IN_DOCKER) $(WALLET_FILE)
	@echo "Building ZeepinChain payload"
	@mkdir -p $@
	@cp docker/Dockerfile $@
	@cp docker/build/bin/ZeepinChain $@
...
docker/build/bin/%: Makefile
	@echo "Building ZeepinChain in docker"
	@mkdir -p docker/build/bin docker/build/pkg
	@$(DRUN) --rm \
		-v $(abspath docker/build/bin):/go/bin \
		-v $(abspath docker/build/pkg):/go/pkg \
		-v $(GOPATH)/src:/go/src \
		-w /go/src/github.com/zeepin/ZeepinChain \
		golang:1.9.5-stretch \
		$(GC)  $(BUILD_NODE_PAR) -o docker/build/bin/ZeepinChain main.go
	@touch $@

docker: Makefile docker/payload docker/Dockerfile 
	@echo "Building ZeepinChain docker"
	@$(DBUILD) -t $(DOCKER_NS)/ZeepinChain docker/payload
	@docker tag $(DOCKER_NS)/ZeepinChain $(DOCKER_NS)/ZeepinChain:$(DOCKER_TAG)
	@touch $@

clean:
	rm -rf *.8 *.o *.out *.6 *exe
	rm -rf ZeepinChain ZeepinChain-* tools docker/payload docker/build
