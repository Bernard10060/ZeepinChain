...
var (
	...
	DisableEventLogFlag = cli.BoolFlag{
		...
		Usage: "If set disableeventlog flag, ZeepinChain will not record event log output by smart contract",
	}
	...
	EnableTestModeFlag = cli.BoolFlag{
		Name:  "testmode",
		Usage: "Using to start a single node test network for ease of development and debug. In testmode, ZeepinChain will start rpc, rest and web socket server, and set default gasprice to 0",
	}
	...
	NetworkIdFlag = cli.UintFlag{
		Name:  "networkid",
		Usage: "Using to specify the network ID. Different networkids cannot connect to the blockchain network. 1=ZeepinChain main net, 2=polaris test net, 3=testmode, and other for custom network",
	...
	RPCDisabledFlag = cli.BoolFlag{
		Name:  "disablerpc",
		Usage: "Using to shut down the rpc server. The ZeepinChain node starts the rpc server by default at startup.",
	}
	...
	AccountPassFlag = cli.StringFlag{
		Name:   "password,p",
		Hidden: true,
		Usage:  "Using to specify the account `<password>` when ZeepinChain node starts. Because the account password entered in the command line is saved in the log, it is easy to leak the password. Therefore, it is not recommended to use this parameter in a production environment.",
	}
	AccountAddressFlag = cli.StringFlag{
		Name:  "account,a",
		Usage: "Using to specify the account `<address|label|index>` when the ZeepinChain node starts. If the account is null, it uses the wallet default account",
	}
